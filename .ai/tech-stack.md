...
- **OpenAI Whisper** (via candle-whisper) provides local speech-to-text conversion for voice commands in English
- **Ollama** containerized service running Llama 3.2:3b model for natural language command interpretation
- **Docker** containerization platform for running Ollama service locally
- **Reqwest** HTTP client for communicating with Ollama API from Rust backend
- **Candle Whisper** (candle-core, candle-whisper) enables native Rust speech-to-text processing
- **HuggingFace Hub** integration for Whisper model downloading and management
