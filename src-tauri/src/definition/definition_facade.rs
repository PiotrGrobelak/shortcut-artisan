...
impl DefinitionFacade {
    ...
    pub async fn save_shortcut(&self, shortcut: ShortcutRequestPayload) -> Result<Shortcut, String> {
        ...
        if let Some(folder_id) = &shortcut.folder_id {
            if let Err(e) = self.folder_repository.add_shortcut_to_folder(folder_id, &shortcut_id) {
    ...
    pub async fn update_shortcut(
        ...
        if existing.folder_id != payload.folder_id {
            if let Some(old_folder_id) = &existing.folder_id {
                if let Err(e) = self.folder_repository.remove_shortcut_from_folder(old_folder_id, id) {
                    log::warn!("Failed to remove shortcut from old folder: {}", e);
                }
            }
            
            if let Some(new_folder_id) = &payload.folder_id {
                if let Err(e) = self.folder_repository.add_shortcut_to_folder(new_folder_id, id) {
